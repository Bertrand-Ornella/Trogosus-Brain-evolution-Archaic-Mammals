...
Trogosus.data<-subset(Trogosus.data, Diet =="Herbivorous" | Diet =="Omnivorous-Carnivorous")
...
#GLS to obtain EQ if need and residuals
GLS_Br_Bo<-gls(Brain ~ Body, data=Trogosus.data)
summary(GLS_Br_Bo) #Final model

#values obtained from model above
a <- 0.5487054 #slope
b <--0.8376703 #intercept

Exp_b<-exp(b)
Exp_b

#Open dataset without log10 -- dataset with just brain and body
Trogosus.data<-read.csv("Trogosus_dataset_final_Eocene.csv", header=T, row.names = 1)
Trogosus.data<-subset(Trogosus.data, Trogosus.data[,5] =="middle Eocene")
Trogosus.data<-subset(Trogosus.data, Diet =="Herbivorous" | Diet =="Omnivorous-Carnivorous")
...
#export dataframe
write.csv(Trogosus.data,'Diet_EV_BM_Trogosus_Middle_Eocene.csv')

############# Diet archaic vs crown groups from the Middle Eocene #################

#Open data with residuals
Trogosus.data<-read.csv("Diet_EV_BM_Trogosus_Middle_Eocene.csv", header=T, row.names = 1)
...
sink("Diet_EV_BM_ANOVA_MiddleEocene_Archaic.txt")
...
#Test normality and homogeneity of the variance
#Not normal but variances are equal - pairwisePermutation test ANOVA
sink("Diet_EV_BM_ANOVA_MiddleEocene_Herbivorous.txt")
shapiro.test(Trogosus.data$residuals) #p_value < 0.05 No normally distributed - levene
...
Trogosus.data<-subset(Trogosus.data, Order =="Perissodactyla" | Order =="Artiodactyla" 
    | Order == "Afrotheria" | Order == "Carnivoramorpha" | Order == "Creodonta" 
    | Order == "Mesonychidae")

#Test normality and homogeneity of the variance
#Not normal but variances are equal - pairwisePermutation test ANOVA
sink("Diet_EV_BM_ANOVA_MiddleEocene_CrownHerb_ArchaicCarni.txt")
shapiro.test(Trogosus.data$residuals) #p_value < 0.05 No normally distributed - levene
#bartlett.test(residuals ~ Diet, data = Trogosus.data) #p-value < 0.05 No equality of variances - welsh
